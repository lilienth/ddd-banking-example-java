...
public class CreditService {
	...
	private Map<CreditAccountNumber, CreditAccount> accountList = new HashMap<>();
	private Map<CreditNumber, Credit> creditList = new HashMap<>();
	private final CreditNumberFactory creditNumberFactory;

	public CreditService() {
		this(new CreditNumberFactory());
	}

	CreditService(CreditNumberFactory creditNumberFactory) {
	...
	public CreditAccount newCreditAccount(CreditAccountNumber creditAccountNumber, CreditCustomer customer, Amount balance, Credit credit) {
		CreditAccount account = new CreditAccount(creditAccountNumber, credit.getAmountOfCredit());
	...
	public CreditAccount grantCredit(CreditNumber creditNumber, CreditAccountNumber creditAccountNumber) {
		require(getCredit(creditNumber).canBeGranted(), "getCredit(creditNumber).canBeGranted()");

		Credit credit = getCredit(creditNumber);
		CreditAccount newCreditAccount = newCreditAccount(creditAccountNumber, credit);
	...
	public Credit getCreditFromAccountNumber(CreditAccountNumber creditAccountNumber) {
		...
		for (Map.Entry<CreditNumber, Credit> entry : creditList.entrySet()) {
			...
			if (account.isPresent() && account.get().getAccountNumber().equals(creditAccountNumber)) {
	...
	public CreditAccount newCreditAccount(CreditAccountNumber creditAccountNumber, Credit credit) {
		CreditAccount account = new CreditAccount(creditAccountNumber, credit.getAmountOfCredit());
		accountList.put(account.getAccountNumber(), account);
	...
	public CreditAccount getCreditAccount(CreditAccountNumber creditAccountNumber) {
		return accountList.get(creditAccountNumber);
	}

	public Set<CreditAccountNumber> getAccountNumberList() {
